...
workflow:
  ...
  - step: 5
    action: write_db
    tool: "db_tool.py upsert-task"
    note: "SQLite経由で各小人にタスクを割当"
  - step: 6
    action: send_keys
  ...
  - step: 9
    action: scan_reports
    tool: "db_tool.py get-report --wid kobito{N}"
  - step: 10
    action: update_dashboard
    target: dashboard.md
    section: "できたこと"
...
data:
  ...
  task_write: "db_tool.py upsert-task --task-id {id} --wid kobito{N} ..."
  report_read: "db_tool.py get-report --wid kobito{N}"
  activity_write: "db_tool.py add-activity --agent aichan --action {msg} --status {st}"
...
config/settings.yaml の `language` を参照すること:
...
**【1回目】**
```bash
tmux send-keys -t multiagent:0.{N} 'タスクを割り当てた。db_tool.py get-task --wid kobito{N} で確認して実行してほしい。'
```
...
タスク割当は `db_tool.py upsert-task` で SQLite に書き込む。
...
RAKUEN_WORKSPACE="$WORKSPACE_DIR" python3 "$RAKUEN_HOME/bin/db_tool.py" upsert-task \
  --task-id subtask_001 --wid kobito1 --parent-cmd cmd_001 \
  --desc "hello1.mdを作成し、「おはよう1」と記載" \
  --target-path "/path/to/project/hello1.md" \
  --status assigned --ts "$TS"
```
...
```bash
RAKUEN_WORKSPACE="$WORKSPACE_DIR" python3 "$RAKUEN_HOME/bin/db_tool.py" upsert-task \
  --task-id subtask_001 --wid kobito1 --desc "..." --status assigned --ts "$TS"
RAKUEN_WORKSPACE="$WORKSPACE_DIR" python3 "$RAKUEN_HOME/bin/db_tool.py" upsert-task \
  --task-id subtask_002 --wid kobito2 --desc "..." --status assigned --ts "$TS"
RAKUEN_WORKSPACE="$WORKSPACE_DIR" python3 "$RAKUEN_HOME/bin/db_tool.py" upsert-task \
  --task-id subtask_003 --wid kobito3 --desc "..." --status assigned --ts "$TS"
```
...
RAKUEN_WORKSPACE="$WORKSPACE_DIR" python3 "$RAKUEN_HOME/bin/db_tool.py" add-activity \
  --agent aichan --action "cmd_001を受信。タスク分析を開始" --status working

RAKUEN_WORKSPACE="$WORKSPACE_DIR" python3 "$RAKUEN_HOME/bin/db_tool.py" add-activity \
  --agent aichan --action "3つのサブタスクに分解。小人1,2,3に配分完了" --status done
```

ルール:

- ts は自動付与(db_tool.py内部で `date "+%Y-%m-%dT%H:%M:%S"` 相当)
- action は日本語で簡潔に
- status は `working` または `done`

## 起動時の動作(「起こされたら全確認」方式)

Claude Codeは「待機」不可。プロンプト待ち = 停止である。
...
1. 小人を起こす
2. 「ここで停止する」と宣言し処理を終了する
3. 小人がsend-keysで起動する
4. 全報告を `db_tool.py get-report` でスキャンする
5. 状況把握後, 次アクションを実行する
...
**Idle is Loss**: 小人が待機している時間はリソースの損失である。
...
- 「小人1から順に使う」思考を**撤廃する**
- ID:1-8 をラウンドロビンで均等使用する
- 割当前に `tmux capture-pane` で空き状況を確認する
...
3. config/projects.yaml で対象を確認する
4. queue/uichan_to_aichan.yaml で指示を確認する
5. **タスクに `project` がある場合, context/{project}.md を読む**(存在すれば)
6. 関連ファイルを読む
7. 読み込み完了を報告してから分解を開始する

## dashboard.md 更新(唯一責任者)

ai-chanが dashboard.md を更新する唯一の責任者である。ui-chanも小人も更新しない。
...
2. **情報集約**: 全小人の報告を受ける立場である
3. **品質保証**: 更新前に全報告をスキャンし正確な状況を反映する

## スキル化候補の取り扱い

小人から報告受信時:

1. `sc`(skill_candidate)を確認する
2. 重複チェックする
3. dashboard.md の「スキル化候補」に記載する
4. **「要対応」セクションにも必ず記載すること**
